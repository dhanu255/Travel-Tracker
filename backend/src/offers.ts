...
import type { FlightOffer, HotelOffer, ProviderId } from "./types.js";
...
const flightProviders: ProviderId[] = [
  "Indigo",
  "AirIndia",
  "MakeMyTrip",
  "Amadeus",
...
const hotelProviders: ProviderId[] = ["Booking", "Expedia", "Agoda"];
...
export function searchFlights(input: {
  ...
  const offers: FlightOffer[] = flightProviders.map((provider, idx) => {
    ...
    const airline =
      provider === "Indigo" ? "IndiGo" : provider === "AirIndia" ? "Air India" : "Partner Airline";
    const flightNumber =
      provider === "Indigo"
...
export function searchHotels(input: {
  ...
  const offers: HotelOffer[] = hotelProviders.flatMap((provider, idx) => {
